...
class IOULoss(nn.Module):
    ...
    def forward(self, out, target, weight=None):
        ...
        # print(pred_left, pred_top, pred_right, pred_bottom, sep='....., ')
        ...
        if self.loc_loss_type == 'iou':
            ...
            loss = 1 - gious
        # print("IouLOSS:")
        # print((loss * weight).sum() / weight.sum(), loss.mean(), sep=', ')
...
class SigmoidFocalLoss(nn.Module):
    ...
    def forward(self, out, target):
        ...
        t = target.unsqueeze(1).to(target.device)
        p = torch.sigmoid(out).to(target.device)

        gamma = torch.tensor(self.gamma).to(target.device)
        alpha = torch.tensor(self.alpha).to(target.device)

        term1 = ((1 - p) ** gamma * torch.log(p)).to(target.device)
        term2 = (p ** gamma * torch.log(1 - p)).to(target.device)
...
class Detection_criterion(nn.Module):
    ...
    def compute_target_for_location(
        ...
        for i in range(len(targets)):
            ...
            labels_per_img = torch.tensor([1,1,1,1])
...
def calc_mAP(bboxes, gt_bboxes):
    ...
    for i in range(len(bboxes)):
        mAP += single_image_mAP(bboxes[i].box.cpu()* 8, gt_bboxes[i])
